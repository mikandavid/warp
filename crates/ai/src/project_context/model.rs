...
impl ProjectContextModel {
    ...
    /// Returns the local file-based global rules for daemon snapshot
    /// publishing. Remote-host rules deliberately remain separate.
    pub fn local_global_rules(&self) -> Vec<ProjectRule> {
        self.global_rules.active_rules().collect()
    }
