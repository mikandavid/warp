...
use crate::proto::{
    ...
    GetDiffStateResponse, GlobalRuleProto, GlobalRulesSnapshot, HomeSkillProto, HomeSkillsSnapshot,
...
#[tokio::test]
async fn home_context_snapshot_pushes_become_client_events() {
    ...
    match event_rx.recv().await.unwrap() {
        ...
        other => panic!("Expected HomeSkillsSnapshotReceived, got {other:?}"),
    }
    match event_rx.recv().await.unwrap() {
        ClientEvent::GlobalRulesSnapshotReceived { snapshot } => {
            assert_eq!(snapshot.rules[0].content, "global");
        }
        other => panic!("Expected GlobalRulesSnapshotReceived, got {other:?}"),
    }
}
